...
export const signup = catchAsync(async (req, res, next) => {
    ...
    res.status(201).json({
        status: "success",
        message:
            "You are signed up. Please use /api/v1/faculty/signin to sign in",
        data: {
            newUser,
        },
    });
...
export const signin = catchAsync(async (req, res, next) => {
    ...
    const user = await Faculty.findOne({ email }).select("+password");
...
export const protect = catchAsync(async (req, res, next) => {
    ...
    const currentUser = await Faculty.findById(decoded.id);
    if (!currentUser) {
        return next(
            new AppError(
